(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
      ...
      fetch("https://pro.ip-api.com/json/?fields=61439&key=3rxukZrZ9IgP9hB")
          ...
          if (matchedCityValue && choicesInstance) {
            try {
              ...
              setTimeout(function () {
                ...
                updateLocationWrapperHeight();
                updateButton();
                updateWebjetUrl(); // Update Webjet URL after location is set
              }, 200);
            } catch (e) {
        ...
        .catch(function (error) {
          ...
          if (choicesInstance) {
            ...
            setTimeout(updateWebjetUrl, 100); // Update Webjet URL after location is set
          }
        });

      locationSelect.addEventListener("change", function () {
        updateButton();
        updateWebjetUrl(); // Update Webjet URL when location changes
    ...
    */
    ...
    // Function to generate the Webjet URL based on current location selection
    function generateWebjetUrl() {
      if (!locationSelect) {
      ...
      // Get date one week from today in YYYYMMDD format
      function getOneWeekFromTodayForURL() {
        var today = new Date();
        var oneWeekFromToday = new Date(
          today.getTime() + 7 * 24 * 60 * 60 * 1000
        ); // Add 7 days
        var year = oneWeekFromToday.getFullYear();
        var month = (oneWeekFromToday.getMonth() + 1)
          .toString()
          .padStart(2, "0");
        var day = oneWeekFromToday.getDate().toString().padStart(2, "0");
        return year + month + day;
      }

      var departDate = getOneWeekFromTodayForURL();
    ...
    // Function to update the Webjet button's href attribute
    function updateWebjetUrl() {
      if (generateWebjet) {
        var webjetUrl = generateWebjetUrl();
        if (webjetUrl) {
          generateWebjet.setAttribute("href", webjetUrl);
          console.log("Updated Webjet button href attribute:", webjetUrl);
        }
      }
    }

    // Initialize the Webjet URL on page load
    updateWebjetUrl();

    // WEBJET GENERATION - Updated for one-way trips with one week from today's date
    if (generateWebjet) {
      generateWebjet.addEventListener("click", function (e) {
        ...
        var webjetUrl = generateWebjetUrl();
        if (webjetUrl) {
          console.log("Opening Webjet URL:", webjetUrl);
          window.open(webjetUrl, "_blank");
        }
      });
    }
  });
